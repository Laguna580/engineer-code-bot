...
def generate_all_timezone_data() -> list:
    ...
    возвращает список словарей с названием, временем и кодом
    """
    data = []
    utc_now = datetime.now(pytz.UTC)
...
def format_codes_table_html(data: list, update_time: datetime) -> str:
    """Форматирует коды в виде красивой таблицы с текущим временем (HTML с <pre>)"""
    
    table = (
        f"🔄 Обновлено: 📅 {update_time.strftime('%d.%m.%Y')} 🕒 {update_time.strftime('%H:%M:%S')} ({config.DISPLAY_TIMEZONE})\n\n"
        f"<pre>"
        f"┌──────────────┬──────────┬─────────┐\n"
...
async def get_or_create_message() -> int:
    ...
    try:
        ...
        text = format_codes_table_html(data, now_local)
...
async def update_codes_message():
    ...
    try:
        ...
        data = generate_all_timezone_data()
        now_local = get_local_time()
        new_text = format_codes_table_html(data, now_local)
...
async def send_welcome(message: Message):
    """Приветственное сообщение (MarkdownV2)"""
    text = (
        ...
        "/reset\\_message — создать новое сообщение \\(если что\\-то сломалось\\)\n\n"
...
async def show_config(message: Message):
    """Показывает текущие настройки бота (MarkdownV2)"""
    text = (
...
async def send_current_time(message: Message):
    """Показывает текущее время (MarkdownV2)"""
    utc_now = datetime.now(pytz.UTC)
...
async def code_for_utc(message: Message):
    """Генерирует код для указанного смещения UTC (HTML)"""
    try:
        ...
        await message.answer("❌ Неверный формат. Используйте число")
    except Exception as e:
...
async def list_timezones(message: Message):
    """Показывает все поддерживаемые часовые пояса (HTML)"""
    text = "🌍 <b>Поддерживаемые часовые пояса:</b>\n\n<pre>"
    text += "┌──────────┬────────┐\n"
    text += "│   Пояс   │ Смещ.  │\n"
    text += "├──────────┼────────┤\n"
...
async def send_codes_table(message: Message):
    """Отправляет таблицу кодов для всех часовых поясов (HTML)"""
    try:
        data = generate_all_timezone_data()
        now_local = get_local_time()
        response = format_codes_table_html(data, now_local)
...
async def main():
    ...
    logger.info(f"📝 Режим таблиц: HTML с <pre>")
    logger.info(f"📝 Режим команд: MarkdownV2 с экранированием")
